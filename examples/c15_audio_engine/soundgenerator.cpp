 ...
 *            on mix amounts (self modulation, cross modulation, feedback modulation),
 *            calculates the samples of the 2 oscillators and 2 shapers
 *            -> reads Envelope signals from the Array in the Singleton Class
*******************************************************************************/

void Soundgenerator::generateSound(float _feedbackSample, uint32_t _vn)
{
    float env_A = ParamSignalData::instance().getSignalValue(_vn, ENV_A);
    float env_B = ParamSignalData::instance().getSignalValue(_vn, ENV_B);
    float env_C = ParamSignalData::instance().getSignalValue(_vn, ENV_C);
    float env_Gate = ParamSignalData::instance().getSignalValue(_vn, ENV_GATE);
